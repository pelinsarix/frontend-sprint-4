...
from classes_auxiliares import NoInterno, NoFolha, SyntaxException, Token
...
if __name__ == "__main__":
    ...
    def command_statement(self):
        if self.tokenCorrente.valor == 'CRIE_FIGURA':
            self.comparar('FUNC_IN')
            self.comparar('LPAR')
            tipo = self.str()
            self.comparar('COMMA')
            ...
            self.comparar('FUNC_IN')
            self.comparar('LPAR')
            ref1 = self.expression()
            self.comparar('COMMA')
            ref2 = self.expression()
            self.comparar('RPAR')
            return NoInterno("colidiu", ref1=ref1, ref2=ref2)
        elif self.tokenCorrente.valor == 'INICIE_CRONOMETRO':
            self.comparar('FUNC_IN')
